...
const excelToJson = (function() {
    ...
    const parseSheet = (sheetData, workbook) => {
        ...
        for (let cell in sheet) {
            ...
            let columnName = snake(columnData);
            if(columnName !== '') columnData = columnName;

            let sheetValue = getSheetCellValue(sheet[cell]);
            if (typeof (sheetValue) === "string" && /\S/.test(sheetValue) === false) {
                sheetValue = null;
            }
            rowData[columnData] = sheetValue;
